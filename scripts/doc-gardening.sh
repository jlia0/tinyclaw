#!/usr/bin/env bash
# doc-gardening.sh — scan for stale or broken documentation and open fix-up PRs.
#
...
# ---------- 3. Crates not mentioned in ARCHITECTURE.md ----------

echo "Checking ARCHITECTURE.md coverage..."

for crate_dir in "$REPO_ROOT"/crates/*/; do
  crate_name="$(basename "$crate_dir")"
  if ! grep -q "$crate_name" "$REPO_ROOT/ARCHITECTURE.md" 2>/dev/null; then
    add_problem "**Missing coverage**: crate \`$crate_name\` is not mentioned in ARCHITECTURE.md."
  fi
done

# ---------- 4. Orphaned doc files not linked from AGENTS.md ----------
...
while IFS= read -r mdfile; do
  ...
  if ! grep -q "$rel" "$REPO_ROOT/AGENTS.md" 2>/dev/null; then
    # Check if it's an index or sub-doc linked from a parent index
